...
if ($stores) {
    ...
    foreach ($stores as $store) {
        ...
        $infowindow_content = str_replace('{name}',get_the_title($store->ID),$infowindow_content);
...
<!-- Show Map -->
<?php if ($map_options['enable']): ?>

	<div id="store_locatore_search_map" style="height: <?php echo $map_options['height'] . $map_options['heightunit']; ?>;width: 74%;position:absolute" class="<?php echo $map_options['listing_position']?>"></div>
        ...
        <div class="store-locator-item-container">
            <div class="wpsl-list-title"><?php _e('Store List','wpmsl')?></div>
                ...
                foreach ($stores as $store) {
                    ...
                    $list_content = '<div class="wpsl-name">'.get_post_meta($store->ID,'store_locator_name',true) . '</div>';
